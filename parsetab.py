...
_lr_productions = [
  ...
  ('stmts -> stmts stmt','stmts',2,'p_stmts','myparser.py',37),
  ('stmts -> empty','stmts',1,'p_stmts','myparser.py',38),
  ('stmt -> assignment','stmt',1,'p_statement','myparser.py',48),
  ('stmt -> for','stmt',1,'p_statement','myparser.py',49),
  ('stmt -> if','stmt',1,'p_statement','myparser.py',50),
  ('stmt -> while','stmt',1,'p_statement','myparser.py',51),
  ('stmt -> expression','stmt',1,'p_statement','myparser.py',52),
  ('if -> IF expression COLON LBRACE stmts RBRACE elif','if',7,'p_if','myparser.py',58),
  ('elif -> ELIF expression COLON LBRACE stmts RBRACE elif','elif',7,'p_elif','myparser.py',64),
  ('elif -> else','elif',1,'p_elif','myparser.py',65),
  ('else -> ELSE COLON LBRACE stmts RBRACE','else',5,'p_else','myparser.py',74),
  ('else -> empty','else',1,'p_else','myparser.py',75),
  ('while -> WHILE expression COLON LBRACE stmts RBRACE','while',6,'p_while','myparser.py',82),
  ('for -> FOR ID IN RANGE LPAREN NUMBER RPAREN COLON LBRACE stmts RBRACE','for',11,'p_for','myparser.py',88),
  ('assignment -> ID ASSIGN expression','assignment',3,'p_assignment','myparser.py',94),
  ('expression -> expression PLUS term','expression',3,'p_expr_operator','myparser.py',100),
  ('expression -> expression MINUS term','expression',3,'p_expr_operator','myparser.py',101),
  ('expression -> expression GE term','expression',3,'p_expr_operator','myparser.py',102),
  ('expression -> expression EQ term','expression',3,'p_expr_operator','myparser.py',103),
  ('expression -> expression NE term','expression',3,'p_expr_operator','myparser.py',104),
  ('expression -> expression LT term','expression',3,'p_expr_operator','myparser.py',105),
  ('expression -> expression LE term','expression',3,'p_expr_operator','myparser.py',106),
  ('expression -> expression GT term','expression',3,'p_expr_operator','myparser.py',107),
  ('expression -> term','expression',1,'p_expr_operator','myparser.py',108),
  ('term -> term TIMES factor','term',3,'p_term','myparser.py',117),
  ('term -> term DIVIDE factor','term',3,'p_term','myparser.py',118),
  ('term -> factor','term',1,'p_term','myparser.py',119),
  ('factor -> NUMBER','factor',1,'p_factor','myparser.py',128),
  ('factor -> ID','factor',1,'p_factor','myparser.py',129),
  ('factor -> LPAREN expression RPAREN','factor',3,'p_factor','myparser.py',130),
  ('empty -> <empty>','empty',0,'p_empty','myparser.py',138),
]
